...
const addPlanForm = useForm({
  bike_quote_uuid: page.props.quote.uuid,
  is_disabled: 0,
  is_create: 1,
  repair_type_comp: '',
...
const setBikePlans = () => {
  ...
  axios
    .get(
      `/bike-insurance-provider-plans?insuranceProviderId=${id}&quoteUuId=${page.props.quote.uuid}`,
    )
    .then(({ data }) => {
...
<template>
  <x-form @submit="creatQuotePlan" :auto-focus="false">
      ...
      <div class="w-full md:w-1/2">
        <div class="flex flex-col gap-4">
          <x-field label="Plan" required>
            <x-select
              v-model="addPlanForm.bike_plan_id"
              :rules="[isRequired]"
    ...
    <div class="flex flex-wrap md:flex-nowrap gap-6 w-full pb-5">
      ...
      <div class="w-full md:w-1/3">
        <x-field label="Bike value" required>
          <x-input
            v-model="addPlanForm.bike_value"
            :rules="[isRequired]"
            class="w-full"
            placeholder="Enter Bike value"
            type="number"
            step="any"
          />
        </x-field>
